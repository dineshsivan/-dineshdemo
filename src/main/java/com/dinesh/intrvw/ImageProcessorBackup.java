...
public class ImageProcessorBackup {
	public static void main(String[] args) throws IOException {
		System.setProperty("http.agent", "Chrome");
		int imgCount = 2;

//		URL url = new URL("E:\\Test Img\\Source.jpg");
		File file = new File("E:\\Source.jpg");
		...
		int First_img_Width = (int) Math.round(image.getWidth() * 0.3);
	 ...
	 * BufferedImage buffimgStamp = new BufferedImage(600, 550,
	 * BufferedImage.TYPE_INT_ARGB); // Create a BufferedImage
	 * buffimgStamp.setRGB(0, 0, 0); // with // the predefined // dimension
	 * BufferedImage loadedStamp = stamp.getBufferedImage(); // Load the image from
	 * file
	 * 
	 * int x = 600 - loadedStamp.getWidth(); // Calculate the x position for
	 * top-right alignment int y = 0; // Calculate the y position for top-right
	 * alignment System.out.println(x+":"+y); Graphics2D g2d =
	 * buffimgStamp.createGraphics(); // Get the Graphics2D object of the
	 * BufferedImage g2d.drawImage(loadedStamp, x, y, null); // Draw the loaded
	 * image at the top-right corner of the BufferedImage g2d.dispose(); // Dispose
	 * the Graphics2D object
	 * 
	 * BufferedImage buffimgSign = new BufferedImage(600, 550,
	 * BufferedImage.TYPE_INT_ARGB); // Create a BufferedImage buffimgSign.setRGB(0,
	 * 0, 0); // with BufferedImage loadedSign = sign.getBufferedImage();
	 * 
	 * int x1 = 0; // Calculate the x position for top-right alignment int y1 =
	 * 550-loadedSign.getHeight(); // Calculate the y position for top-right
	 * alignment System.out.println(x1+":"+y1); Graphics2D g2d2 =
	 * buffimgSign.createGraphics(); // Get the Graphics2D object of the
	 * BufferedImage g2d2.drawImage(loadedSign, x1, y1, null); // Draw the loaded
	 * image at the top-right corner of the BufferedImage g2d2.dispose(); // Dispose
	 * the Graphics2D object
	 * 
	 * ImageIO.write(buffimgStamp, "png", new
	 * File("E:\\Test Img\\buffimgStamp.png")); ImageIO.write(buffimgSign, "png",
	 * new File("E:\\Test Img\\buffimgSign.png"));
	 */

	/*
	 * MarvinImage stampToAdd = new MarvinImage(buffimgSign); MarvinImage signToAdd
	 * = new MarvinImage(buffimgStamp);
	 * MarvinPluginCollection.blackAndWhite(stampToAdd,150);
	 * MarvinPluginCollection.boundaryFill(stampToAdd.clone(), stampToAdd, 0, 0,
	 * Color.white, 150); stampToAdd.setColorToAlpha(0, 0xFFFFFFFF);
	 * MarvinPluginCollection.alphaBoundary(stampToAdd, 5);
	 * MarvinPluginCollection.blackAndWhite(signToAdd,150);
	 * MarvinPluginCollection.boundaryFill(signToAdd.clone(), signToAdd, 0, 0,
	 * Color.white, 150); signToAdd.setColorToAlpha(0, 0xFFFFFFFF);
	 * MarvinPluginCollection.alphaBoundary(signToAdd, 5);
	 * 
	 * List<MarvinImage> images = new ArrayList<MarvinImage>();
	 * images.add(stampToAdd); images.add(signToAdd);
	 * 
	 * MarvinImage combine =signToAdd.clone();
	 * MarvinPluginCollection.alphaBoundary(combine, 5);
	 * 
	 * //MarvinPluginCollection.mergePhotos(images, combine, 38);
	 * MarvinPluginCollection.combineByTransparency(signToAdd, stampToAdd, combine,
	 * 0, 0, 0);
	 * 
	 * MarvinImageIO.saveImage(combine, "E:\\Test Img\\combine.png");
	 */
